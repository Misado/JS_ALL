...
//console.log("selectItem: "+selectItem+typeof(selectItem));
...
document.getElementById("submutBtn").onclick = function(){
    ...
    switch(selectItem){
        ...
        for ( var i=1; i < wsPlayer.length; i++){
            ...
            console.log(wsPlayer[i].Speler+" 跟 "+wsPlayer[0].Speler+" 差距： "+diffScore+" 分");
            //document.getElementById("diffScroreResult").textContent += wsPlayer[i].Speler+" 跟 "+wsPlayer[0].Speler+" 差距： "+diffScore+" 分";
            document.write("<table style=\"border: 1px solid #000\"><tr><td>"+wsPlayer[i].Speler+" 跟 "+wsPlayer[0].Speler+" 差距： "+"</td><td>"+diffScore+" 分</td></tr></table>");
            //document.getElementById("diffScroreResult").textContent += "<table style=\"border: 1px solid #000\"><tr><td>"+wsPlayer[i].Speler+" 跟 "+wsPlayer[0].Speler+" 差距： "+"</td><td>"+diffScore+" 分</td></tr></table>";
        }
        break;
        case 2:
            console.log("SWITCHselectItem: "+selectItem);
            console.log(objAll[0].Country);
            for ( var i=0; i < objAll.length; i++){
                console.log("------");
                console.log("objAll: "+objAll[i].Country+" "+objAll[i].Count);
                document.write("<table style=\"border: 1px solid #000\"><tr><td>"+objAll[i].Country+"</td><td>"+objAll[i].Count+" 人</td></tr></table>");
            }
            break;
        case 3:
            //document.getElementById("playerList").textContent = "test";
            var countryInput = document.getElementById("countryInput").value;
            console.log("countryInput: "+countryInput);
            document.getElementById("playerList").textContent = countryInput+"有： ";
            for ( var i=0; i < wsPlayer.length; i++){
                if ( wsPlayer[i].Land == countryInput){
                    console.log(countryInput+": "+wsPlayer[i].Speler);
